...
class cap_genie:
    ...
    """
    get_files: None --> list
    -- Gets all FASTQ files from directory structure (regular or nested)
    * @param [out] files (list) - List of FASTQ file paths
    ** Detects and handles both regular folders with FASTQ files and nested directory structures
    """
    def get_files(self):
        # Define supported FASTQ extensions
        fastq_extensions = (".fastq", ".fq", ".fastq.gz", ".fq.gz")
        
        if not self.session_name:
            # Check if this is a regular folder (contains FASTQ files directly) or nested structure
            direct_fastq_files = [f for f in os.listdir(self.nested_dir) 
                                if f != ".DS_Store" and f.endswith(fastq_extensions)]
            
            subdirectories = [f for f in os.listdir(self.nested_dir) 
                            if f != ".DS_Store" and os.path.isdir(os.path.join(self.nested_dir, f))]
            
            # Check if subdirectories contain FASTQ files
            nested_fastq_found = False
            for subdir in subdirectories:
                subdir_path = os.path.join(self.nested_dir, subdir)
                subdir_fastq = [f for f in os.listdir(subdir_path) 
                              if f.endswith(fastq_extensions)]
                if subdir_fastq:
                    nested_fastq_found = True
                    break
            
            if direct_fastq_files and not nested_fastq_found:
                # Regular folder case - FASTQ files are directly in the main folder
                print(color.BOLD + "Processing Regular Folder" + color.END)
                print(f"Found {len(direct_fastq_files)} FASTQ files in: {self.nested_dir}")
                print("FASTQ files found:")
                for i, file in enumerate(direct_fastq_files, 1):
                    print(f"-- {file} [{i}]")
                
                # Use the folder name as the single directory
                self.dirs = [os.path.basename(self.nested_dir)]
                print(f"Processing all FASTQ files in folder: {self.dirs[0]}")
                
            elif nested_fastq_found:
                # Nested folder case - original behavior
                file_map = {}
                idx = 1
                print(color.BOLD + "Processing Nested Folders" + color.END)
                ...
                for subdir in subdirectories:
                    ...
                    subdir_fastq = [f for f in os.listdir(subdir_path) 
                                  if f.endswith(fastq_extensions)]
                    if subdir_fastq:  # Only show folders that contain FASTQ files
                        print(f"-- {subdir} [contains {len(subdir_fastq)} FASTQ files] [{idx}]")
                        file_map[str(idx)] = os.path.join(self.nested_dir, subdir)
                        idx += 1
                
                if not file_map:
                    print("No folders containing FASTQ files found!")
                    return
                    
                print("All FASTQ files in selected folders will be processed.")
                
                folders = input("Select which folders to process (numbers separated by spaces, or 'all'): ")
                if folders == "all":
                    self.dirs = [os.path.basename(v) for k,v in file_map.items()]
                else:
                    # Gets folder basenames
                    selected_folders = folders.split()
                    self.dirs = [os.path.basename(file_map[folder]) for folder in selected_folders if folder in file_map]
            else:
                print("No FASTQ files found in the specified directory or its subdirectories!")
                print("Supported extensions: .fastq, .fq, .fastq.gz, .fq.gz")
                return
        else:
            # Session mode - check for both regular and nested structures
            direct_fastq_files = [f for f in os.listdir(self.nested_dir) 
                                if f != ".DS_Store" and f.endswith(fastq_extensions)]
            
            if direct_fastq_files:
                # Regular folder case
                self.dirs = [os.path.basename(self.nested_dir)]
            else:
                # Nested folder case
                subdirectories = [dir for dir in os.listdir(self.nested_dir) 
                                if dir != ".DS_Store" and os.path.isdir(os.path.join(self.nested_dir, dir))]
                # Filter to only include directories that contain FASTQ files
                self.dirs = []
                for subdir in subdirectories:
                    subdir_path = os.path.join(self.nested_dir, subdir)
                    subdir_fastq = [f for f in os.listdir(subdir_path) 
    ...
    ** Filters low-quality reads from FASTQ files
    ...
    def denoise_files(self, instance):
        ...
        for dir in self.dirs:
            # Check if this is a regular folder case (dir name matches the base folder name)
            if dir == os.path.basename(self.nested_dir):
                # Regular folder case - files are directly in nested_dir
                source_dir = self.nested_dir
                files_to_process = [f for f in os.listdir(source_dir) 
                                  if f.endswith(fastq_extensions)]
            else:
                # Nested folder case - files are in subdirectory
                source_dir = os.path.join(self.nested_dir, dir)
                files_to_process = [f for f in os.listdir(source_dir) 
                                  if f.endswith(fastq_extensions)]
    ...
    ** Orchestrates the entire CAPGENIE pipeline workflow
    ...
    def run_pipeline(self):
        ...
        # Ensure the folder containing the enrichment file is processed first
        if self.enrichment_file and dirs_to_use:
            try:
                normalized_enrichment_path = os.path.normpath(self.enrichment_file)
                enrichment_dirname = os.path.dirname(normalized_enrichment_path)
                enrichment_dir_basename = os.path.basename(enrichment_dirname) if enrichment_dirname else None

                prioritized = None
                remaining = []

                for candidate_dir in dirs_to_use:
                    # For denoised runs, dirs_to_use contains absolute paths to denoised_<dir>
                    if self.quality_threshold:
                        if os.path.normpath(candidate_dir) == os.path.normpath(enrichment_dirname):
                            prioritized = candidate_dir
                            continue
                    else:
                        # Non-denoised runs: dirs_to_use contains basenames
                        if enrichment_dir_basename and os.path.basename(candidate_dir) == enrichment_dir_basename:
                            prioritized = candidate_dir
                            continue
                    remaining.append(candidate_dir)

                if prioritized is not None:
                    dirs_to_use = [prioritized] + remaining
            except Exception:
                # If any issue occurs determining the priority directory, fall back to original order
                pass
