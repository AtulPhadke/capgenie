...
class enrichment:
    ...
    ** Calculates enrichment factors for peptide data
    ...
    def calc_enrichment(self, pre_insert, session_folder, files, data_directory, instruction_name):
        ...
        # Handle potential duplicates by combining counts for same peptides
        pre_insert_combined = {}
        for peptide, decimal in zip(pre_insert_dict.Peptide, pre_insert_dict.Decimal):
            if peptide in pre_insert_combined:
                pre_insert_combined[peptide] += decimal
            else:
                pre_insert_combined[peptide] = decimal
        pre_insert_dict = {x:y for x,y in pre_insert_combined.items() if y != 0}
        ...
        for file in files:
            if os.path.basename(pre_insert).replace(".fastq", "") not in file:
                ...
                # Handle potential duplicates by combining counts for same peptides
                obj_combined = {}
                for peptide, decimal in zip(file_dict.Peptide, file_dict.Decimal):
                    if peptide in obj_combined:
                        obj_combined[peptide] += decimal
                    else:
                        obj_combined[peptide] = decimal
                obj_dict = obj_combined
