...
std::unordered_map<char, char> makeTranslationMap(const std::string& from, const std::string& to) {
    ...
    translationMap.reserve(from.size());  // Pre-allocate memory
...
std::string translateString(const std::string& input, const std::unordered_map<char, char>& translationMap) {
    ...
    translated.reserve(input.size());  // Pre-allocate memory
    
    for (char& c : translated) {
        auto it = translationMap.find(c);
        if (it != translationMap.end()) {
            c = it->second;  // Use iterator instead of .at() for better performance
        }
    }
        ...
        if (upstream_mismatches <= 4) {
            try {
                if (downstream_mismatches <= 4) {
                    std::string extracted_read = safe_substring(line, mer(7), mer(28));
                    
                    // Only include reads with more than 3 characters
                    if (extracted_read.length() > 3) {
                        if (result.dircheck == "fwd") {
                            result.forward_reads.push_back(extracted_read);
                        } else {
                            result.reverse_reads.push_back(extracted_read);
                        }
                    }
                }
            } catch (...) {
