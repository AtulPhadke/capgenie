...
class spreadsheet:
    ...
    ** Saves processed data to Excel spreadsheet format
    ...
    def save_file(self, pkl_file_path, file, data_directory, instruction_link, avg_file=False, barcode=True):
        ...
        if not avg_file:
            df = pd.read_pickle(os.path.join(pkl_file_path, data_directory, f"{file_ext}{file}").replace(".fastq.gz", ".pkl").replace(".fq.gz", ".pkl").replace(".fq", ".pkl").replace(".fastq", ".pkl"))
            
            # Filter out blank or null peptide rows
            if "Peptide" in df.columns:
                df = df[df["Peptide"].notna()]
                df = df[df["Peptide"].astype(str).str.strip() != ""]
            ...
            if "Peptide" in df.columns and "Count" in df.columns:
                ...
                df = df.reset_index()
            df.to_excel(os.path.join(self.sheets_dir, data_directory, f"{file_ext}{file}").replace(".fastq.gz", ".xlsx").replace(".fq.gz", ".xlsx").replace(".fq", ".xlsx").replace(".fastq", ".xlsx"))
        else:
            df = pd.read_pickle(os.path.join(pkl_file_path, data_directory, file).replace(".fastq.gz", ".pkl").replace(".fq.gz", ".pkl").replace(".fq", ".pkl").replace(".fastq", ".pkl"))
            
            # Filter out blank or null peptide rows (for average files, peptides are in the index)
            ...
            df.to_excel(os.path.join(self.sheets_dir, data_directory, file).replace(".fastq.gz", ".xlsx").replace(".fq.gz", ".xlsx").replace(".fq", ".xlsx").replace(".fastq", ".xlsx"))
